...
class GANLossDict(nn.Module):
    ...
    def __init__(self, gan_mode: str, target_real_label: float=1.0, target_fake_label: float=0.0) -> None:
        ...
        LSGAN needs no sigmoid. vanilla GANs will handle it with BCEWithLogitsLoss.
        """
        super(GANLossDict, self).__init__()
        self.register_buffer('real_label', torch.tensor(target_real_label))
        self.register_buffer('fake_label', torch.tensor(target_fake_label))
        self.gan_mode = gan_mode
        if gan_mode == 'lsgan':
    ...
    def __call__(self, netD, imgs, target_is_real, subset=None):
        """Calculate loss given Discriminator's output and grount truth labels.
        Parameters:
            prediction (tensor) - - tpyically the prediction output from a discriminator
            target_is_real (bool) - - if the ground truth label is for real images or fake images
        ...
        for mod in subset:
            nb_voxels = np.prod(imgs[mod].shape)
            prediction = netD[mod](imgs[mod])
            loss[mod] = 0.
            if self.gan_mode in ['lsgan', 'vanilla']:
                target_tensor = self.get_target_tensor(prediction, target_is_real)
                loss[mod] = nb_voxels*self.loss(prediction, target_tensor)
            elif self.gan_mode == 'wgangp':
                if target_is_real:
                    loss[mod] =-nb_voxels*prediction.mean()
                else:
                    loss[mod] = nb_voxels*prediction.mean()
...
class GANLossDictV2(nn.Module):
    ...
    def __init__(self, gan_mode: str, target_real_label: float=1.0, target_fake_label: float=0.0) -> None:
        ...
        LSGAN needs no sigmoid. vanilla GANs will handle it with BCEWithLogitsLoss.
        """
        super(GANLossDictV2, self).__init__()
        self.register_buffer('real_label', torch.tensor(target_real_label))
        self.register_buffer('fake_label', torch.tensor(target_fake_label))
        self.gan_mode = gan_mode
        if gan_mode == 'lsgan':
    ...
    def __call__(self, netD, imgs, target_is_real, subset=None):
        ...
        for mod in subset:
            ...
            if self.gan_mode in ['lsgan', 'vanilla']:
                target_tensor = self.get_target_tensor(prediction, target_is_real)
                loss[mod] = self.loss(prediction, target_tensor)
            elif self.gan_mode == 'wgangp':
                if target_is_real:
                    loss[mod] = -prediction.mean()
                else:
                    loss[mod] = prediction.mean()
        output_loss = sum(loss.values()) / len(loss.values())
        return output_loss